...
SketchyDynamics::PhysicsJointType PhysicsJoint::getType() const
{
	return _pjt;
}
...
{
	_poly = new AnimatedPolygon(VV_Static, "SD_Joint" + toString(ulong(id)));
	...
	_poly->setUserType(SKETCHYDYNAMICS_POLYGON_UTYPE_PHYJOINT);
...
SketchyDynamics::JointAnchorsSituation PhysicsJointRevolute::checkAnchorsSituation(bool updateVisual) 
{
...
{
	SKETCHYDYNAMICS_ASSERT(_selected && "Cannot setPosition on an unselected joint");
	_poly->Polygon::setPosition(position);
}

void PhysicsJointRevolute::translate( const Vector2& amount )
{
	SKETCHYDYNAMICS_ASSERT(_selected && "Cannot translate an unselected joint");
	_poly->Polygon::translate(amount);
}

void PhysicsJointRevolute::rotateAroundPoint( float angle, const Vector2& rotationPoint )
{
	SKETCHYDYNAMICS_ASSERT(_selected && "Cannot rotateAroundPoint an unselected joint");
...
SketchyDynamics::JointAnchorsSituation PhysicsJointWeld::checkAnchorsSituation(bool updateVisual)
{
	JointAnchorsSituation jas = JAS_NOT_MOVED;
	Vector2 jointPos = _poly->getPosition();
	// TODO: improve anchor check (sometimes A and B anchors can be different)
...
{	
	...
	std::string jointName = "SD_Joint" + toString(ulong(id));
	...
	_zigZagPoly->setUserType(SKETCHYDYNAMICS_POLYGON_UTYPE_PHYJOINT);
	...
	_circlePolyA->setUserType(SKETCHYDYNAMICS_POLYGON_UTYPE_PHYJOINT);
	_circlePolyA->setUserData(this);
	_circlePolyB->setUserType(SKETCHYDYNAMICS_POLYGON_UTYPE_PHYJOINT);
...
SketchyDynamics::JointAnchorsSituation PhysicsJointDistance::checkAnchorsSituation(bool updateVisual)
{
	JointAnchorsSituation jas = JAS_NOT_MOVED;
...
{
	SKETCHYDYNAMICS_ASSERT(_selected && "Cannot setPositions on an unselected joint");
...
SketchyDynamics::Vector2 PhysicsJointDistance::getPositionA() const
{
	return _circlePolyA->getPosition();
}

SketchyDynamics::Vector2 PhysicsJointDistance::getPositionB() const
{
