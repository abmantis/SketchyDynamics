...
SketchyDynamics::AABB SubPolygon::getWorldAABB( bool bestFit, Matrix3 transformMatrix ) const
{
	AABB aabb;
	if(bestFit)
	{
...
Polygon::Polygon(VertexVariance vv /*= VV_Static*/, std::string name /*= ""*/, CoordinateSystem cs /*= CS_Scene*/) :
	...
	_userData			(NULL),
	_userType			(SKETCHYDYNAMICS_POLYGON_UTYPE_NONE),
	_visible			(true),
	_alpha				(1.0f)
{
}
...
SketchyDynamics::AABB Polygon::getTransformedAABB( bool bestFit ) const
{
	AABB aabb;
	if(bestFit)
	{
...
SketchyDynamics::ulong Polygon::getUserType() const
{
	return _userType;
}
...
SketchyDynamics::uint Polygon::getSubPolygonCount() const
{
