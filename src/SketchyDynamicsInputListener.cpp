...
#include "SketchyDynamicsPhysicsQueryCallback.h"
...
void MainInputListener::initInputListener()
{
	...
	_transformCenterIndicator = new Polygon(VV_Static, "SD_transform_center_indicator");
	...
	_transformLineIndicator = new Polygon(VV_Static, "SD_transform_line_indicator");
	...
	_selectedBodiesAABBPoly = new Polygon(VV_Static, "SD_selected_bodies_aabb");
	...
	Material* destructAreaMat = MaterialManager::getSingletonPtr()->createMaterial("SD_destruction_area", "../../../textures/destruct_area.png");	
	_destructionArea = new AnimatedPolygon(VV_Static, "SD_destruction_area");
...
//	_destructionArea->setVisible(false);
	...
	_gestureMaterial = MaterialManager::getSingletonPtr()->createMaterial("SD_gesture_material", Color(0.0f, 0.0f, 8.0f, 1.0f));
			...
			if(_isLeftMouseDown)
			{
				...
				case IS_NONE:
				{
					...
					if(callback._firstPolygon != nullptr)
					{
						...
						{							
							if( callback._firstPolygon->getUserType() == SKETCHYDYNAMICS_POLYGON_UTYPE_PHYJOINT)
							{
							...
							else if( callback._firstPolygon->getUserType() == SKETCHYDYNAMICS_POLYGON_UTYPE_GUESSESLIST)
							{
...
{	
	...
	{		
		...
		FirstObjectSceneQueryCallback callback(sceneMousePos);
		_renderer->queryScene(sceneMousePos, &callback);
		if(callback._firstPolygon != nullptr && callback._firstPolygon->getUserType() == SKETCHYDYNAMICS_POLYGON_UTYPE_GUESSESLIST)
		{
...
void MainInputListener::startDrawingGesture( Vector2 startPoint )
{
	...
	_gesturePolygon = new Polygon(VV_Stream, "SD_gesture");
...
{	
	...
	{
		_physicsMgr->_simulationPaused_sketchydynamics = true;
		...
		for (int i = 0; i < recGestCount; ++i)
		{
			...
			else if(gestureName.compare("Alpha") == 0)
			{
				...
				{
					SKETCHYDYNAMICS_LOG_WARNING("Can't find alpha intersection?!");
...
void MainInputListener::hideGuessesList()
{
	...
	{				
		...
		_guessesListVisible = false;
		_physicsMgr->_simulationPaused_sketchydynamics = false;
	}
}
