...
namespace SketchyDynamics
{
	...
	#ifndef NDEBUG
	#define SKETCHYDYNAMICS_ASSERT(expr) if (!(expr)) __sketchydynamics_assert__(__FILE__, __LINE__, #expr, true); 
	#else 
	#define SKETCHYDYNAMICS_ASSERT(expr) if (!(expr)) __sketchydynamics_assert__(__FILE__, __LINE__, #expr, false); 
	#endif
